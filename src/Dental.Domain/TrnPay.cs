...
{
    [Serializable]
    [Table(Name = TrnPayColumn.TABLE_NAME)]
    public class TrnPay : BaseDomain<TrnPay>, ICommonFunctions<TrnPay>
    {
        ...
        [ColumnAttribute(Name = TrnPayColumn.PAY_DATE, CanBeNull = true)]
        ...
        public TrnPay GetByPrimaryKey()
        {
            Table<TrnPay> table = GetTable();

            TrnPay item = table.Single(d => (d.OfficeCd == this.OfficeCd && d.PurchaseSeq  == this.PurchaseSeq && d.SupplierOutsourceCd == this.SupplierOutsourceCd));
            if (item != null)
                item.Detach<TrnPay>();
            return item;
        }
