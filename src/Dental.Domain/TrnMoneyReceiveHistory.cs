...
{
    [Serializable]
    [Table(Name = TrnMoneyReceiveHistoryColumn.TABLE_NAME)]
    public class TrnMoneyReceiveHistory : BaseDomain<TrnMoneyReceiveHistory>, ICommonFunctions<TrnMoneyReceiveHistory>
    {
        ...
        public TrnMoneyReceiveHistory GetByPrimaryKey()
        {
            Table<TrnMoneyReceiveHistory> table = GetTable();

            TrnMoneyReceiveHistory item = table.Single(d => (d.OfficeCd == this.OfficeCd && d.PayDate == this.PayDate && d.BillCd == this.BillCd));
            if (item != null)
                item.Detach<TrnMoneyReceiveHistory>();
            return item;
        }
    }
}
